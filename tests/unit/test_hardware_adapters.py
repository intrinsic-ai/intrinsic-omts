...
class HardwareAdaptersTest(absltest.TestCase):
  ...
  def test_dio_cnc_machine(self):
    ...
    mock_solution.skills.ai.intrinsic.dio_set_output = dio_set_mock
    mock_solution.skills.ai.intrinsic.dio_wait_for_input = dio_read_mock
    ...
    dio_read_mock.assert_called_with(
      block_name="standard_in", indices=[7], values=[True], timeout=30.0
    )
  ...
  def test_dio_cnc_machine_single_pin_and_device_name(self):
    ...
    # When device_name is "ur_module", adio should be omitted to let
    # SBL auto-resolve.
    ...
    dio_read_mock.assert_called_with(
      block_name="standard_in", indices=[7], values=[True], timeout=10.0
    )
  ...
  def test_orbbec_vision_build_perception_with_retries(self):
    mock_solution = mock.MagicMock()
    ...
    pipeline = vision.build_perception_and_spawn_task(
      max_tries=3,
      retry_delay_sec=1.5,
    )
    self.assertIsInstance(pipeline, bt.Sequence)
    self.assertEqual(len(pipeline.children), 2)
    retry_node = pipeline.children[0]
    self.assertIsInstance(retry_node, bt.Retry)
    self.assertEqual(retry_node.max_tries, 3)
    self.assertIsInstance(retry_node.child, bt.Sequence)
    self.assertEqual(len(retry_node.child.children), 2)
    self.assertIn("Capture", retry_node.child.children[0].name)
    self.assertIn("Estimate", retry_node.child.children[1].name)
    self.assertIsNotNone(retry_node.recovery)
    self.assertIn("Dwell", retry_node.recovery.name)

  def test_orbbec_vision_build_perception_without_retries(self):
    mock_solution = mock.MagicMock()
    mock_action = mock.MagicMock(spec=bt.ActionBase)
    mock_solution.skills.ai.intrinsic.capture_images.return_value = mock_action
    mock_solution.skills.ai.intrinsic.estimate_pose_multi_view.return_value = (
    ...
    pipeline = vision.build_perception_and_spawn_task(max_tries=1)
    self.assertIsInstance(pipeline, bt.Sequence)
    self.assertEqual(len(pipeline.children), 2)
    acq_node = pipeline.children[0]
    self.assertNotIsInstance(acq_node, bt.Retry)
    self.assertIsInstance(acq_node, bt.Sequence)
    self.assertEqual(len(acq_node.children), 2)
