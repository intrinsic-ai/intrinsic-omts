...
class MoveToJointTest(absltest.TestCase):
  def test_list_available_joint_configs(self):
    ...
    mock_robot.joint_configurations.__getitem__.side_effect = lambda k: (
      mock_config_home if k == "home" else mock_config_view
    )
    mock_world.ur_module = mock_robot

    configs = list_available_joint_configs(
      mock_world, arm_part_name="ur_module"
    )
    expected = [
  ...
  def test_move_robot_to_joint_named_success(self):
    ...
    self.assertEqual(task.name, "Move ur_module to home")
  ...
  def test_move_robot_to_joint_positions_success(self):
    ...
    (task,), _ = mock_solution.executive.run.call_args
    self.assertEqual(
      task.name, "Move ur_module to [0.0, -1.57, 1.57, -1.57, -1.57, 0.0]"
    )
