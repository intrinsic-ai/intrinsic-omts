...
#######################################
# Applies scene updates (.pbtxt), aligns the tool frame, or seeds the scene.
# Globals:
#   INCTL_ADDR
# Arguments:
#   action: 'apply', 'align', or 'seed'.
#   extra_args: File paths or trailing tool options.
#######################################
cmd_world() {
  if [ $# -gt 0 ] && { [ "$1" = "-h" ] || [ "$1" = "--help" ]; }; then
    echo "Usage: $0 world <apply|align|seed> [files|extra_args...]"
    exit 0
  fi
  if [ $# -lt 1 ]; then
    die "Usage: $0 world <apply|align|seed> [extra_args...]"
  fi
  local action="$1"
  shift
  case "$action" in
    apply)
      ...
      ;;
    align)
      run_target //tools/world:align_tool_frame -- "$@"
      ;;
    seed)
