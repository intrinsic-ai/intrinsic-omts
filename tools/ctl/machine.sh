...
cmd_machine() {
  ...
  case "$normalized_action" in
    ...
    close)
      ...
      ;;
    sync|sync_world)
      normalized_action="sync_open"
      ;;
    open_door|close_door|open_vise|close_vise|trigger_cycle|sync_open|sync_closed|prep) ;;
    wait_cycle)
...
cmd_prep_machine() {
  ...
  echo "Preparing cell, resetting world, retracting robot, and" \
