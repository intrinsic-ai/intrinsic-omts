...
stop_app() {
  ...
  (cd "$SCRIPT_DIR" && bazel run -c opt @intrinsic-core//intrinsic/config:empty_application -- --address="$INCTL_ADDR" --operation_mode real || true)
...
Workcell Commands:
  ...
  world apply [files]                 Apply scene updates (.pbtxt)
