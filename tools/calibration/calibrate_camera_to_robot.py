...
def build_camera_world_updates(
  ...
  if moving_camera:
    ...
    if not stationary_camera_poses:
      raise ValueError(
          'stationary_camera_poses must be provided when moving_camera is'
          ' False.'
      )
...
def apply_camera_updates_to_world(
  ...
  print('[✓] Successfully applied updated camera transform to live ObjectWorld!')
...
def main(argv) -> None:
  ...
  if not export_path and _APPLY_TO_WORLD.value is None:
    ...
    if save_choice == 'y':
      default_path = 'configs/lab_bb_01_orbbec_gemini.updates.pbtxt'
