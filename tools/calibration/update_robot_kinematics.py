...
from intrinsic.world.service.robot_calibration import robot_update_service_pb2_grpc
...
def update_robot_kinematics(
  ...
  try:
    ...
    logging.info(
        "Successfully updated robot kinematics in world '%s' for resource"
        " '%s'.",
        world_id,
        resource_id,
    )
...
def main(argv: Sequence[str]) -> None:
  ...
  try:
    ...
    if matches_after:
      ...
      logging.warning(
          "Update completed but verification check reported mismatch for"
          " '%s'.",
          resource_id,
      )
  finally:
