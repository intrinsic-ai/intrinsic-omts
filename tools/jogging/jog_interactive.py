...
_PART_NAME = flags.DEFINE_string(
    "part_name", "", "Specific part name to control in ICON. If empty, uses the first controllable part."
)
...
def main(argv: Sequence[str]) -> None:
  ...
  print(f"Connecting to ICON at {_HOST.value}:{_PORT.value} (instance: '{_INSTANCE.value}')...")
      ...
      while True:
        try:
          ...
          if not (0 <= joint_idx < ndof):
            print(f"Error: Invalid joint index. Must be between 0 and {ndof-1}.")
            continue

          print(f"\n--- Live control for joint {joint_idx} ---")
          print("Use Left/Right arrow keys to jog by 0.01 rad. Press 'q' or 'x' to return.")
