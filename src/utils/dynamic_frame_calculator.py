...
def _extract_target_estimate(params: Any) -> data_types.Pose3 | None:
  """Extracts the highest-confidence FoundationPose estimate or direct pose."""
  ...
  if isinstance(raw, (list, tuple, Sequence)) and not isinstance(
    raw, (str, bytes)
  ):
  ...
  if (
    hasattr(params, "pos_x")
    and hasattr(params, "pos_y")
    and hasattr(params, "pos_z")
  ):
    cam_q = data_types.Quaternion(
      [
        getattr(params, "ori_x", 0.0),
        getattr(params, "ori_y", 0.0),
        getattr(params, "ori_z", 0.0),
        getattr(params, "ori_w", 1.0),
      ]
    )
    return data_types.Pose3(
      data_types.Rotation3(cam_q),
...
def _sync_frame(
  ...
  existing = (
    set(str(f) for f in parent_obj.list_frames()) if has_list_frames else set()
  )
  frame_exists = frame_name in existing or (
    not has_list_frames and hasattr(parent_obj, frame_name)
  )
  if frame_exists:
...
def calculate_and_update_dynamic_frames(context: Any, params: Any) -> None:
  ...
  if camera_obj is None:
    raise ValueError(f"Camera object '{camera_name}' not found in world.")

  root_t_camera = _resolve_camera_transform(world, parent_obj, camera_name)
  root_t_target = (
    root_t_camera * cam_pose if root_t_camera is not None else cam_pose
  )
  ...
  if target_id:
    target_obj = getattr(world, target_id, None)
