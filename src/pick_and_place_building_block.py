...
_ENABLE_CNC = flags.DEFINE_bool(
    "enable_cnc",
    False,
    "Whether to include CNC machine operations (door open, vise open) in pick"
    " and place.",
)
...
_CNC_DOOR_CLOSE_PIN = flags.DEFINE_integer(
    "cnc_door_close_pin",
    3,
    "Digital output pin index to close CNC enclosure door.",
)
_CNC_DEVICE_NAME = flags.DEFINE_string(
    "cnc_device_name",
    "",
    "Device name owning CNC DIO pins (empty to auto-resolve ADIO service).",
)
...
def run_pick_and_place_loop(
    ...
    enable_cnc: bool = False,
    cnc_door_open_pin: int = 2,
    cnc_door_close_pin: int | None = 3,
    cnc_device_name: str = "",
  ...
  if enable_cnc and machine is None:
    if mock_hardware:
      ...
      machine = DioCncMachine(
          ...
          door_close_pin=cnc_door_close_pin,
          device_name=cnc_device_name or None,
      )
  ...
  # Open CNC enclosure door if enabled
  if enable_cnc and machine is not None:
  ...
  while num_cycles <= 0 or cycle_count < num_cycles:
    ...
    tree = build_building_block_pick_place_tree(
        robot=robot,
        gripper=gripper,
        machine=machine if enable_cnc else None,
        parent_object=parent_object,
...
def main(argv: Sequence[str]) -> None:
  ...
  run_pick_and_place_loop(
      ...
      enable_cnc=_ENABLE_CNC.value,
      cnc_door_open_pin=_CNC_DOOR_OPEN_PIN.value,
      cnc_door_close_pin=_CNC_DOOR_CLOSE_PIN.value,
