...
name (see `src.behaviors.grasp_planning`).
"""
...
class MoveItGraspPlanner(GraspPlannerInterface):
  ...
  def build_plan_grasps_task(
    ...
    Args:
      ...
      surfaces: Object surfaces to generate grasp candidates on. Defaults to the
        top face (+Z) only, which keeps the candidate count low and the approach
        vertical.
    ...
    Raises:
      ValueError: If no candidate objects are supplied, or if num_rotations is
        not positive.
    """
    ...
    kwargs: dict[str, Any] = {
      ...
      "retract_dist_m": self._retract_dist_m,
...
class MockGraspPlanner(GraspPlannerInterface):
  ...
  def build_plan_grasps_task(
    ...
    return bt.Task(
      action=bt.PythonScript(
        function_body=(
          f'print("[MockGraspPlanner] Planned grasp for'
