...
def main():
    ...
    parser.add_argument("--patchelf", required=True, help="Path to patchelf binary")
    parser.add_argument("--so-file", required=True, help="Path to foundationpose_cpp.so")
    parser.add_argument("--wheel", action="append", default=[], help="Wheel file paths")
    ...
    with tempfile.TemporaryDirectory() as stage_dir:
        ...
        site_packages_dir = os.path.join(stage_dir, "lib", "python3.12", "site-packages")
        ...
        with tarfile.open(output_path, "w:gz", format=tarfile.GNU_FORMAT) as tf:
            for item in sorted(os.listdir(stage_dir)):
                tf.add(os.path.join(stage_dir, item), arcname=item, filter=reset_tarinfo)
