...
examples (run through Bazel as `bazel run
  ...
  moveit_plan_grasp_and_move --target_object=raw_stock_2x3x5 --plan_only --surfaces=0,1,4,5
  ...
  # bazel run //tools/world:apply_scene_updates -- --files configs/omts/raw_stock_in_vise.updates.pbtxt
  moveit_plan_grasp_and_move --target_object=raw_stock_2x3x5 --surfaces=0,1,4,5
...
def resolve_target_objects(raw_values: Sequence[str] | None) -> list[str]:
  ...
  if not raw_values:
    raise ValueError(
      ...
      " (e.g. --target_object=raw_stock_2x3x5)."
    )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
  ...
  parser.add_argument(
    ...
    metavar="NAME[,NAME...]",
    help=(
      "Object World object name to plan grasps for (e.g. raw_stock_2x3x5)."
  ...
  parser.add_argument(
    ...
    help=(
      ...
      " raw_stock_2x3x5 to bar end-cap grasps on its 75 mm Y axis."
    ),
  )
  parser.add_argument(
    "--num_rotations",
